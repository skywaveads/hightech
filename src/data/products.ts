...
export async function initDB() {
  try {
    ...
    if (!fs.existsSync(dbPath)) {
      ...
      fs.writeFileSync(dbPath, JSON.stringify(initialProducts, null, 2));
...
export async function deleteProduct(id: string): Promise<boolean> {
  try {
    ...
    // Save to file
    fs.writeFileSync(dbPath, JSON.stringify(filteredProducts, null, 2));
...
// Initialize database when first imported
initDB(); 